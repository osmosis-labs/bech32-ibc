...
BECH32IBC_VERSION := 02cd685503e7b8535b91d4a2cf70b6d91ec5f28b
...
ifeq (cleveldb,$(findstring cleveldb,$(BECH32IBC_BUILD_OPTIONS)))
  build_tags += gcc
endif
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=bech32ibc \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=bech32ibcd \
...
ifeq (,$(findstring nostrip,$(BECH32IBC_BUILD_OPTIONS)))
  ldflags += -w -s
endif
ldflags += $(LDFLAGS)
...
draw-deps:
	...
	@goviz -i ./cmd/bech32ibcd -d 2 | dot -Tpng -o dependency-graph.png
...
build-docker-bech32ibcdnode:
	$(MAKE) -C networks/local 

# Run a 4-node testnet locally
localnet-start: build-linux build-docker-bech32ibcdnode # localnet-stop
	@if ! [ -f $(BUILDDIR)/node0/bech32ibcd/config/genesis.json ]; \
	then docker run --rm -v $(BUILDDIR):/bech32ibcd:Z osmosis-labs/bech32ibcdnode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; \
	fi
	docker-compose up -d
...
.PHONY: all build-linux install format lint \
	...
	setup-transactions setup-contract-tests-data start-bech32ibc run-lcd-contract-tests contract-tests \
	test test-all test-build test-cover test-unit test-race \
	benchmark \
	build-docker-bech32ibcdnode localnet-start localnet-stop \
	docker-single-node